...
const LEVELS = [
  { number: 1, label: 'שלב 1 — בס סטטי / יורד' },
  { number: 2, label: 'שלב 2 — + תנועה צעדית' },
  { number: 3, label: 'שלב 3 — מזכיר V→I (סקרציוני)' },
  { number: 4, label: 'שלב 4 — בס מינורי יורד' },
  { number: 5, label: 'שלב 5 — מעורב מז\'ור / מינור' },
  { number: 6, label: 'שלב 6 — קונטור בס מורכב' },
  { number: 7, label: 'שלב 7 — קפיצות בס' }
];
...
const ExerciseV1 = () => {
  const generateQuestion = useCallback((level) => {
    ...
    return {
      id: Date.now(),
      progression: correct,
      options: all.map(p => ({ id: p.id, label: 'בס: ' + p.bassDegrees.join('-') })),
